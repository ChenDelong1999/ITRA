...
AVALIABLE_IMAGE_MODEL_BUILDER = ['openclip', 'chineseclip', 'torchvision', "torchhub", "vitae"]
...
def get_model(args):
    ...
    if args.text_model_builder=='openclip':
        ...
        if "coca" in args.text_model:
            text_backbone = CLIP_model.text
        else:
        ...
        if args.adapter is not None:
            raise RuntimeError(f'Adapter {args.adapter} is not avaliable for {args.text_model_builder} model!')

    elif args.text_model_builder == 'remoteclip':
        ...
        args.text_width, args.text_dim = text_backbone.text_projection.size()
        text_backbone.layers = open_clip.get_model_config(args.text_model)['text_cfg']['layers']
        cache_dir = os.path.join('../training', args.cache_dir, 'remoteclip', args.pth_name)
        checkpoint = torch.load(cache_dir, map_location='cpu')
        sd = checkpoint["state_dict"]
        sd["module.text_backbone.logit_scale"] = sd.pop("module.logit_scale")
        sd = {k[len('module.text_backbone.'):] : v for k, v in sd.items() if "text_backbone" in k}

        msg = text_backbone.load_state_dict(sd, strict=False)
        if is_master(args):
            logging.info(msg)
        ...
        if 'Taiyi' in args.text_model:
            # Taiyi model need classification heads, using AutoModel will skip loading them.
    ...
    if args.image_model_builder=='openclip':
        ...
        if "convnext" in args.image_model:
            image_backbone.output_dim = 512
        else:
            args.image_dim = image_backbone.output_dim

        if "convnext" not in args.image_model:
    ...
    elif args.image_model_builder == 'remoteclip':
        CLIP_model, preprocess_train, preprocess_val = open_clip.create_model_and_transforms(
            model_name=args.image_model,
            pretrained=args.image_model_tag if args.pretrained_image_model else '',
            precision=args.precision,
        ...
        checkpoint = torch.load(cache_dir, map_location='cpu')
        sd = checkpoint["state_dict"]
        sd = {k[len('module.image_backbone.'):] : v for k, v in sd.items() if "image_backbone" in k}

        msg = image_backbone.load_state_dict(sd, strict=False)
        if is_master(args):
            logging.info(msg)
        ...
        image_backbone = CLIP_model.visual
        args.image_dim = image_backbone.output_dim
    # elif args.image_model_builder == 'lavis':
    #     from lavis.models import load_model_and_preprocess
    #     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    #     # loads BLIP caption base model, with finetuned checkpoints on MSCOCO captioning dataset.
    #     # this also loads the associated image processors
    #     model, preprocess_train, preprocess_val = load_model_and_preprocess(name="blip_caption", model_type="base_coco", is_eval=True,
    #                                                          device=device)
        ...
        image_backbone = torch.hub.load(args.image_model_tag, args.image_model, pretrained=args.pretrained_image_model)
        ...
        if 'resnet' in args.image_model:
            ...
            if 'swav' in args.image_model_tag or 'barlowtwins' in args.image_model_tag:
                image_backbone.output_dim = image_backbone.fc.weight.shape[1]
                image_backbone.fc = torch.nn.Identity()
        ...
        image_backbone.to(device=args.device)
        preprocess_train = training.transforms.get_preprocess(image_resolution=args.image_resolution, is_train=True,
                                                              aug=args.augmentation)
        preprocess_val = training.transforms.get_preprocess(image_resolution=args.image_resolution, is_train=False,
                                                            aug=None)
        # preprocess_train, preprocess_val = training.transforms.preprocess_train, training.transforms.preprocess_val


    elif args.image_model_builder == 'vitae':

        if args.image_model == 'vitae_nc_base_win_rvsa':
            from model.ViTAE.vitae_nc_win_rvsa import ViTAE_NC_Win_RVSA
            image_backbone = ViTAE_NC_Win_RVSA(img_size=args.image_resolution, use_abs_pos_emb=True)

        if args.pretrained_image_model:
            cache_dir = os.path.join('../training', args.cache_dir, 'vitae', args.pth_name)
            checkpoint = torch.load(cache_dir, map_location='cpu')
            print("Load pre-trained checkpoint from: %s" % cache_dir)
...
class ItraModel(nn.Module):
    ...
    # text backbone
        ...
        if args.prompt:
            assert args.text_model_builder=='openclip' # CoOp style prompt only supports OpenCLIP model
    ...
    def encode_text(self, texts, projection=False, use_pooler=True):
        with self.text_context():
            if self.text_model_builder in ['openclip', 'remoteclip']:
