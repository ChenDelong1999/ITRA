...
torchrun --nproc_per_node 8 -m training.main \
    ...
    --image-model 'ViT-L-14-336' --pretrained-image-model --image-model-builder 'OpenCLIP' --image-head-n-layers 0 \
    --text-model 'ViT-L-14-336' --pretrained-text-model  --text-head-n-layers 0 \
    --distiller 'InfoNCE'\
    --report-to tensorboard --logs 'logs/save_pretrained' --copy-codebase --name 'ViT-L-14-336-openai'