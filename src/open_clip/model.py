...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, projection=False):
        image_feature = self.visual(image)
        if projection:
            image_feature = self.visual.image_projection_head(image_feature)
    ...
    def encode_text_pretrained(self, text):
        ...
        #text_feature = text_feature @ self.text_projection # [batch_size, embed_dim]
        return text_feature
    ...
    def forward(self, image, text):
        ...
        image_features = self.encode_image(image, projection=True)
    ...
    def forward_projection(self, image, text):
        ...
        image_features_projected = self.visual.image_projection_head(image_features)
        image_features = F.normalize(image_features, dim=-1)
