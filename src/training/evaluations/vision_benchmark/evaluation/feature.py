...
def load_custom_ic_model(config):
    ...
    if ext == 'pth':
        ...
        print('=> Unknown model file, skip loading weights')
        return model
...
def get_model(config, feature_type='image'):
    ...
    if model_name in dir(torchvision.models):
        ...
        # Update embed dim
        config.defrost()
        config.MODEL.SPEC.EMBED_DIM = model.image_dim
        config.freeze()
...
def extract_feature(model, data_loader, config):
    ...
    visual_backbone = model.image_backbone if hasattr(model, 'image_backbone') else model
    ...
    config.freeze()
    print( "model_info['n_visual_params']",  model_info['n_visual_params'])
    ...
    with torch.no_grad():
        for batch in tqdm(data_loader, f'Extracting features with model {config.MODEL.NAME}.'):
            ...
            try:
                outputs = model(x, projection=True)
            except:
                outputs = model(x)
