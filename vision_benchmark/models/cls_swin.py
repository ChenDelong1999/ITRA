...
def get_cls_model(opt, **kwargs):
    swin_spec = opt['MODEL']['SPEC']

    swin = SwinTransformer(
        img_size=opt['MODEL']['IMAGE_SIZE'][0],
        patch_size=swin_spec['PATCH_SIZE'],
        in_chans=swin_spec['IN_CHANS'],
        num_classes=opt['MODEL']['NUM_CLASSES'],
        embed_dim=swin_spec['EMBED_DIM'],
        ...
        qk_scale=swin_spec.get('QK_SCALE', None),
        drop_rate=swin_spec['DROP_RATE'],
        drop_path_rate=swin_spec['DROP_PATH_RATE'],
        ape=swin_spec['APE'],
        patch_norm=swin_spec['PATCH_NORM'],
        use_checkpoint=False,
        layer_scale=swin_spec.get('LAYER_SCALE', False)
    )

    if opt['MODEL']['INIT_WEIGHTS']:
        swin.init_weights(
            opt['MODEL']['PRETRAINED'],
            opt['MODEL']['PRETRAINED_LAYERS'],
            opt['VERBOSE']
        )
