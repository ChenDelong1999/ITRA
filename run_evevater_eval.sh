...
# output directory
# output_dir=./outputs
# output_dir=$log_dir/$mode

if [ $mode = zeroshot ]
then
  output_dir=$log_dir/ELEVATER_evaluation/$mode
else
    if [ $num_shots = -1 ]
    then
        output_dir=$log_dir/ELEVATER_evaluation/$mode
    else
        output_dir=$log_dir/ELEVATER_evaluation/$num_shots-shot
    fi
fi
...
# cd vision_benchmark
cd src/training/evaluations/vision_benchmark
log_dir=$log_dir
...
if [[ "$mode" = "linear_probe" ]]; then
    ...
    python commands/zeroshot.py \
        ...
        --dataset_root $dataset_root \
        --ds resources/datasets/$dataset.yaml \
        --model resources/model/$model_cfg.yaml MODEL.CLIP_FP32 $use_fp32 DATASET.ROOT $output_dir/datasets OUTPUT_DIR $output_dir/log KNOWLEDGE.WORDNET.USE_HIERARCHY $use_wordnet_hierachy KNOWLEDGE.WORDNET.USE_DEFINITION $use_wordnet_definition KNOWLEDGE.WIKITIONARY.USE_DEFINITION $use_wiktionary_definition KNOWLEDGE.GPT3.USE_GPT3 $use_gpt3 KNOWLEDGE.AGGREGATION.NUM_GPT3_ITEMS $use_gpt3_count TEST.MODEL_FILE $model_ckpt
else
